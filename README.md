...
附阿拉伯語母語者發音、複數、詞根與文化小知識。每張卡下方還有一個
**土耳其語對照區塊**（土耳其官方標準語），可以順便把兩個語言一起學。
...
每張單字卡下方會多一個區塊，用**土耳其官方標準語**（İstanbul Türkçesi，以 TDK
...
**卡片只留給「可以點的東西」**：全 app 只有兩種東西是卡片 —— 列表裡的一列，
以及單字卡的米哈拉布主視覺。純粹的內容分組一律用 `.band`：上下兩條 1px 分隔線
界定一段，標題貼在上面那條線的上方（標題自己的裝飾線用 `:has(+ .band)` 收起來）。
這樣就不會出現「卡片裡再放一層卡片」——原本設定頁、教室設定、小知識、土耳其語
區塊都是卡片包卡片，邊框和陰影疊了兩層，內容反而被框架蓋過去。

**為什麼土耳其語用綠松石色**：turquoise 這個字本來就是「土耳其的（石頭）」，
İznik 磁磚也是這個顏色。用它當土耳其語區塊的主色，和上面阿拉伯語內容的黃銅金
一眼就分得開，又不必另外引進一個跟整套色盤打架的顏色（`--teal` 本來就在裡面）。
區段本身不上底色，只把上下兩條分隔線和文字換成綠松石。
